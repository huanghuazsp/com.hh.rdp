...
public class ListTreeJspTemplate
{
  ...
  protected final String TEXT_13 = "-deleteTreeByIds'," + NL + "\t\t\tid : treeNode.id," + NL + "\t\t\tcallback : function(result) {" + NL + "\t\t\t\tif (result.success) {" + NL + "\t\t\t\t\t$('#centerdiv').disabled('请选择要编辑的树节点或添加新的数据！');" + NL + "\t\t\t\t}" + NL + "\t\t\t}" + NL + "\t\t});" + NL + "\t}" + NL + "\tfunction treeClick(treeNode) {" + NL + "\t\t$('#centerdiv').undisabled();" + NL + "\t\tvar iframe = window.frames[iframeId];" + NL + "\t\tiframe.callback = function(object) {" + NL + "\t\t\ttreeNode.name = object.text;" + NL + "\t\t\ttreeNode.isParent = object.leaf==1?0:1;" + NL + "\t\t\t$.hh.tree.updateNode('tree', treeNode);" + NL + "\t\t\t$.hh.tree.getTree('tree').refresh();" + NL + "\t\t}" + NL + "\t\tiframe.findData(treeNode.id);" + NL + "\t}" + NL + "</script>" + NL + "</head>" + NL + "<body>" + NL + "\t<div xtype=\"border_layout\">" + NL + "\t\t<div config=\"render : 'west'\">" + NL + "\t\t\t<div xtype=\"toolbar\" config=\"type:'head'\">" + NL + "\t\t\t\t<span xtype=\"button\" config=\"onClick: doAdd ,text:'添加'\"></span> <span" + NL + "\t\t\t\t\txtype=\"button\"" + NL + "\t\t\t\t\tconfig=\"onClick: $.hh.tree.doUp , params:{treeid:'tree',action:'";
